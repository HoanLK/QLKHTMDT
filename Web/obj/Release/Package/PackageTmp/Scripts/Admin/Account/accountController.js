admin.controller("accountController", ['$scope', '$http', '$cookies', '$window', '$location', function ($scope, $http, $cookies, $window, $location) {
    //---VAR---
    $scope.currentUser = {};
    ...
    //Đơn vị
    $scope.donvis = [];
    var apiDonVi = "/API/DonViAPI";

    //---POPUP---
    //Account
    //modify
    $scope.modifyAccount = false;
    $scope.titlePopupModifyAccount = "Sửa thông tin tài khoản";
    ...
    $scope.gridAccounts = {
        bindingOptions: {
            dataSource: 'accounts',
            'columns[4].lookup.dataSource': 'donvis',
        },
    ...
    function GetAccountByRole(role) {
        $http.get('/Account/GetAccountByRole?RoleId=' + role)
            .then(
            function success(response) {
                $scope.accounts = angular.copy(response.data);
            },
            function error(response) {
                toastr.error("Không lấy được danh sách Account");
            }
            );
    }
    //Add
    ...
    $scope.SaveAccount = function (e) {
        ...
        $http.get(apiAccount + "/" + $scope.account.Id)
            .then(function success(response) {
                ...
                account.DienThoai = $scope.account.DienThoai;
                account.DonVi_ID = $scope.account.DonVi_ID;
                //Lưu qua API
    ...
    function GetAllDonVi() {
        $http.get(apiDonVi)
            .then(function success(response) {
                $scope.donvis = angular.copy(response.data);
            }, function error(response) {
                toastr.error("Không lấy được danh sách Đơn vị");
            });
    }
